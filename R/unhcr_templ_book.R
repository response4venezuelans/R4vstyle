#' @name unhcr_templ_book
#' @rdname unhcr_templ_book
#' @title UNHCR Book template for bookdown
#'
#' Loads additional style and template file
#'
#' @references https://github.com/grimbough/msmbstyle
#'
#' @details \code{unhcr_templ_book()} is used to create output split
...
tufte_html_dependency = function(features, variant) {
  list(htmltools::htmlDependency(
    ...
    src = template_resources('tufte_html',
                             package = 'tufte'),
    stylesheet = c(sprintf('tufte-%s.css', features),
                   'tufte.css',
                   if (variant != 'default')
                     sprintf('%s.css', variant)
    )
  ))
}
...
#' unhcr_html_dependency()
#' }
unhcr_html_dependency = function() {
  list(#bookdown:::jquery_dependency(),
    htmltools::htmlDependency('BootSideMenu', '2.0',
                   src = template_resources('unhcr-book', 'BootSideMenu',
                                            package = 'unhcRstyle'),
                   script = "BootSideMenu.js",
                   stylesheet = "BootSideMenu.css"),
    htmltools::htmlDependency('unhcr_css', version = '0',
                   src = template_resources('unhcr-book', package = 'unhcRstyle'),
                   stylesheet = 'unhcr_book.css')
  )
}
...
unhcr_build_chapter = function(
  head, toc, chapter, link_prev, link_next, rmd_cur, html_cur, foot
) {

  ## insert script for solution toggle
  ## we put it before the unhcr_book.css & after jQuery
  unhcr_css_line <- max(str_which(head, "unhcr_book.css\""))
  head[unhcr_css_line] <- paste(toggle_script(), head[unhcr_css_line], sep = "\n")
  ...
  paste(c(
    head,
    '<div class="navbar-fixed-top unhcr-header" id="navbar">',
    '<div class="row-fluid">',
    toc,
    '</div>',
    '</div>',
